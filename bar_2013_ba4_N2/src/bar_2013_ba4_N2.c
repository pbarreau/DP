 ...
 *\DESCRIPTION         Router.
 *
 * This file forms the basic Jenie application for the router
 ...
 *
 * This file also makes calls to functions in Keyboard.c to initialise,
 * read and transmit data from the keyboard.
 */
/* CHANGE HISTORY
 *
 * $Log: Router.c,v $
...
PUBLIC void vJenie_CbConfigureNetwork (void)
{
	...
	vNetwork_ConfigureNetwork(E_JENIE_ROUTER, NETWORK_MAX_CHILDREN, FALSE);
	/* Configure services */
	...
	uint8 *pu8Data)		/**< Pointer to data */
{
	...
	if (u8Service == NETWORK_SERVICE)
	{
		...
		vKeyboard_Rx(u16Length, pu8Data);
	}
}
